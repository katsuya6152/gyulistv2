// 分析血統データ取得ユースケース（DDD+関数型+オニオンアーキテクチャ）

import type { CalfRepository } from "../../../domain/contracts/calf-repository";
import type { AnalyticsFilter, PedigreeAnalysis } from "../../../domain/entities/analytics";
...
// 入力パラメータ
export interface GetAnalyticsPedigreeInput {
  readonly farmId: string;
  readonly startDate?: string;
  readonly endDate?: string;
  readonly analysisType?: "sire" | "maternal" | "full";
...
export class GetAnalyticsPedigreeUseCase {
  constructor(
    private readonly calfRepository: CalfRepository,
    private readonly analyticsService: AnalyticsService
  ) {}

  async execute(input: GetAnalyticsPedigreeInput): Promise<Result<PedigreeAnalysis[], UseCaseError>> {
    try {
      // バリデーション
      ...
      // 血統分析を計算
      const pedigreeAnalysis = this.analyticsService.calculatePedigreeAnalysis(filteredCalves);

      // 制限を適用
      const limitedResults = input.limit ? pedigreeAnalysis.slice(0, input.limit) : pedigreeAnalysis;
      ...
      console.error("GetAnalyticsPedigreeUseCase error:", error);
      return {
        success: false,
        error: {
          code: "UNKNOWN_ERROR",
          message: "血統分析データの取得に失敗しました",
  ...
  private validateInput(input: GetAnalyticsPedigreeInput): Result<true, UseCaseError> {
    if (!input.farmId) {
      return {
        success: false,
        error: {
